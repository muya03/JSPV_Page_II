...
const caAreas = Array.from(new Set(EXECUTIVE_FULL.map((m) => m.area)));
...
export default function Partit() {
  ...
  return (
    <Layout crumbs={[{ label: t.partit.crumb }]}>
      <PageHero title={t.partit.title} subtitle={t.partit.subtitle} />

      {/* Història */}
      <section className="bg-white">
        <div className="container-page py-16 md:py-20">
          <SectionHeading
            eyebrow={t.partit.historiaEyebrow}
            title={t.partit.historiaTitle}
            intro={t.partit.historiaIntro}
          />

          <ol className="mt-12 relative border-l-2 border-border ml-2 pl-8 space-y-10">
            {t.data.history.map((m, i) => (
      ...
      <section className="bg-[hsl(var(--surface))] border-y border-border">
        <div className="container-page py-16 md:py-20">
          <SectionHeading eyebrow={t.partit.valorsEyebrow} title={t.partit.valorsTitle} />
      ...
      <section className="bg-white">
        <div className="container-page py-16 md:py-20">
          <SectionHeading
            eyebrow={t.partit.executivaEyebrow}
            title={t.partit.executivaTitle}
            intro={t.partit.executivaIntro}
          />

          <div className="mt-12 space-y-12">
            {caAreas.map((caArea) => {
              const members = EXECUTIVE_FULL.filter((m) => m.area === caArea);
              const areaLabel = t.data.executiveAreas[caArea] ?? caArea;
              return (
                <div key={caArea}>
